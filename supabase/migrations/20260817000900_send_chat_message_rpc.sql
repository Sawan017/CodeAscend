...
BEGIN
  ...
  IF my_chat ? 'blockedUsers' AND my_chat->'blockedUsers' @> to_jsonb(p_receiver_id::text) THEN
    RAISE EXCEPTION 'Cannot send message to blocked user';
  END IF;

  -- 2. Check if the receiver has blocked me
  SELECT data INTO receiver_chat FROM public.profiles WHERE user_id = p_receiver_id AND key = 'chat';
  ...
  IF NOT EXISTS (
    ...
    WHERE (user_id1 = my_uid AND user_id2 = p_receiver_id)
       OR (user_id2 = my_uid AND user_id1 = p_receiver_id)
  ) THEN
  ...
  new_msg := jsonb_build_object(
    'id', p_msg_id,
    'conversationId', (
      SELECT CASE WHEN my_uid < p_receiver_id THEN my_uid::text || '_' || p_receiver_id::text
                  ELSE p_receiver_id::text || '_' || my_uid::text END
    ),
  ...
  -- We use an explicit UPDATE to avoid conflicts
