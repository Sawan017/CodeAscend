-- Fix send_chat_message RPC to properly call notify_user with the correct signature
...
BEGIN
  ...
  IF target_settings ? 'blockedUsers' AND jsonb_typeof(target_settings->'blockedUsers') = 'array' THEN
      IF target_settings->'blockedUsers' @> to_jsonb(my_uid::text) THEN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Make notification creation non-blocking
  END;
