import { analyzeRepository } from './src/lib/github-analyzer.js';
import { SKILL_REGISTRY } from './src/data/learningData.js';
import { DETECTORS } from './src/lib/analyzer/index.js';
...
async function runTests() {
  ...
  try {
    ...
    // TEST 2
    mockFiles = [{ path: 'App.spec.tsx', content: 'const u: string = "abc"; useState(0); test("w", () => {});' }];
    ...
    // TEST 3 (SQL)
    mockFiles = [{ path: 'query.sql', content: 'SELECT * FROM users;' }];
    result = await analyzeRepository('test', 'repo', 'token');
    const sqlDomains = result.evidences.filter(e => e.skill === 'SQL').map(e => e.domain);
    passed = sqlDomains.includes('databases') && sqlDomains.includes('data-engineering') && sqlDomains.includes('data-science');
    verify("3. Multi-domain skill (SQL)", passed, sqlDomains);

    // TEST 4
    DETECTORS.push({
      name: 'FakeSkill', extensions: ['.fake'], stripComments: (c: string) => c,
    ...
    passed = result1.evidences.length > 0 && result2.evidences.length > 0 && result1.evidences[0].fingerprint === result2.evidences[0].fingerprint;
    verify("6. Same evidence analyzed twice", passed, result1.evidences.length > 0 ? result1.evidences[0].fingerprint : null);
